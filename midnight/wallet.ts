// Wallet construction + sync-state restore for PrivatePass.
// Mirrors the proven Midnight Hello architecture and keeps the frontend in src/
// untouched while the SDK logic remains isolated here.
...
export async function createWallet(opts: CreateWalletOptions): Promise<WalletContext> {
  ...
  const saved: PersistedWalletState = opts.restore === false ? {} : loadWalletState(opts.network, { cwd: opts.cwd });
