// Midnight wallet/network configuration for PrivatePass.
// This is intentionally separate from the React frontend in src/; the UI stays
// a localStorage demo while this module handles real Midnight contract wiring.
...
export function getOrCreateWallet(network: NetworkId, opts: SeedOptions = {}): WalletCredentials {
  ...
  if (envSeed) {
    ...
    if (!SEED_HEX_RE.test(hex)) {
      throw new Error('MIDNIGHT_WALLET_SEED must be 32-128 hex characters.');
    }
...
export function formatWalletBackupNotice(wallet: WalletCredentials, network: NetworkId): string | null {
  ...
  return [
    ...
    `  It is also saved to ${STATE_FILE_NAME} (gitignored).`,
    '',
  ].join('\n');
}
